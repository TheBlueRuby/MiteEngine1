...
void playMidi()
{
	...
	if(sfont_id == FLUID_FAILED)
	{
		int sfontError = MessageBoxW(
				...
				(LPCWSTR)"Mite Engine 1 Soundfont ERROR",
	...
	//Midi file loading
	int midiLoad = fluid_player_add(player, "./D_MUSIC.mid");
	...
	if(midiLoad == FLUID_FAILED)
	{
		int midiError = MessageBoxW(
				...
				(LPCWSTR)"Mite Engine 1 MIDI ERROR",
						 MB_ICONERROR | MB_YESNO
				  );
    	switch (midiError)
    	{
...
{ 
	...
	glutInitWindowPosition (glutGet(GLUT_SCREEN_WIDTH)/2-windowX/2,glutGet(GLUT_SCREEN_HEIGHT)/2-windowY/2);
